...
const OWNED_STORAGE_PREFIX = 'aion2_owned_';
...
function renderMaterials(aggMats) {
  ...
  for (let [matName, totalQty] of sortedMaterials) {
    ...
    const ownedQty = loadOwnedQty(matName) || 0;
    const buyQty = Math.max(totalQty - ownedQty, 0);
    const savedPrice = loadPrice(matName);
    
    html += `
      <div class="material-row fade-in" style="animation-delay:${idx * 0.05}s" data-material="${escapeHtml(matName)}" data-total-qty="${totalQty}">
        ...
        <div class="material-info">
          ...
          <div class="material-detail">已擁有 ${ownedQty.toLocaleString()}，預計需補 ${buyQty.toLocaleString()}</div>
        </div>
        <div class="material-qty">
          <span>需求</span>
          <strong>${totalQty.toLocaleString()}</strong>
          <small>需補 ${buyQty.toLocaleString()}</small>
        </div>
        <div class="owned-input-wrapper">
          <span class="owned-currency">📦</span>
          <input type="text" class="owned-qty-input" placeholder="已有" 
                 data-material="${escapeHtml(matName)}" data-total-qty="${totalQty}" 
                 value="${ownedQty > 0 ? ownedQty.toLocaleString() : ''}" oninput="onOwnedQtyInput(this)" id="owned-${idx}">
        </div>
        ...
        <div class="material-subtotal ${savedPrice ? 'has-value' : ''}" id="subtotal-${idx}">
          ${savedPrice ? formatKinah(savedPrice * buyQty) : '—'}
        </div>
      </div>
    `;
    idx++;
...
function onPriceInput(input) {
  ...
  syncMaterialRow(row, { persist: false });
  calculateTotal();
}

function onOwnedQtyInput(input) {
  const raw = input.value.replace(/[^0-9]/g, '');
  const ownedQty = parseInt(raw) || 0;
  const matName = input.dataset.material;
  const row = input.closest('.material-row');

  if (raw) {
    input.value = ownedQty.toLocaleString();
  }

  if (ownedQty > 0) {
    saveOwnedQty(matName, ownedQty);
  } else {
    removeOwnedQty(matName);
  }
...
function getMaterialRowState(row) {
  if (!row) return null;

  const totalQty = parseInt(row.dataset.totalQty) || 0;
  const ownedInput = row.querySelector('.owned-qty-input');
  const priceInput = row.querySelector('.price-input');
  const subtotalEl = row.querySelector('.material-subtotal');
  const qtyEl = row.querySelector('.material-qty');
  const detailEl = row.querySelector('.material-detail');

  const ownedQty = normalizeQuantity(ownedInput ? ownedInput.value : row.dataset.ownedQty);
  const buyQty = Math.max(totalQty - ownedQty, 0);
  const price = normalizeQuantity(priceInput ? priceInput.value : row.dataset.price);
...
function syncMaterialRow(row, options = {}) {
  if (!row) return;

  const { persist = true, keepNumericFormatting = true } = options;
  const state = getMaterialRowState(row);
  if (!state) return;

  if (state.ownedInput) {
    if (state.ownedQty > 0) {
      state.ownedInput.value = keepNumericFormatting ? state.ownedQty.toLocaleString() : state.ownedQty.toString();
    } else {
      state.ownedInput.value = '';
    }
  }

  if (state.priceInput) {
    if (state.price > 0) {
      state.priceInput.value = keepNumericFormatting ? state.price.toLocaleString() : state.price.toString();
    } else {
  ...
  if (state.qtyEl) {
    state.qtyEl.innerHTML = `<span>需求</span><strong>${state.totalQty.toLocaleString()}</strong><small>需補 ${state.buyQty.toLocaleString()}</small>`;
  }

  if (state.detailEl) {
    state.detailEl.textContent = `已擁有 ${state.ownedQty.toLocaleString()}，預計需補 ${state.buyQty.toLocaleString()}`;
  }

  if (state.subtotalEl) {
    if (state.price > 0) {
      state.subtotalEl.textContent = formatKinah(state.price * state.buyQty);
      state.subtotalEl.classList.add('has-value');
    } else {
      state.subtotalEl.textContent = '—';
      state.subtotalEl.classList.remove('has-value');
    }
  }

  if (persist) {
    const matName = row.dataset.material;
    if (state.price > 0) {
      savePrice(matName, state.price);
    } else {
      removePrice(matName);
    }

    if (state.ownedQty > 0) {
      saveOwnedQty(matName, state.ownedQty);
    } else {
      removeOwnedQty(matName);
    }
  }
...
function calculateTotal() {
  ...
  if (infoEl) {
    if (rows.length === 0) {
      ...
      infoEl.textContent = `輸入材料市價以試算扣除已有數量後的總和 (${filledCount}/${rows.length})`;
    }
  }
}
...
function clearAllPrices() {
  ...
  document.querySelectorAll('.material-row').forEach(row => {
    syncMaterialRow(row, { persist: false, keepNumericFormatting: false });
  });
...
function exportReport() {
  ...
  document.querySelectorAll('.material-row').forEach(row => {
    ...
    const matName = row.dataset.material;
    const subtotal = state.price * state.buyQty;
    
    total += subtotal;
    text += `${matName} | 需求: ${state.totalQty.toLocaleString()} | 已有: ${state.ownedQty.toLocaleString()} | 需補: ${state.buyQty.toLocaleString()}`;
    if (state.price > 0) {
      text += ` | 單價: ${state.price.toLocaleString()} | 小計: ${subtotal.toLocaleString()}\n`;
    } else {
      text += ` | 金額待補\n`;
    }
  });
...
function saveOwnedQty(name, qty) { try { localStorage.setItem(OWNED_STORAGE_PREFIX + name, qty.toString()); } catch(e){} }
function loadOwnedQty(name) { try { const v = localStorage.getItem(OWNED_STORAGE_PREFIX + name); return v ? parseInt(v) : null; } catch(e){ return null; } }
function removeOwnedQty(name) { try { localStorage.removeItem(OWNED_STORAGE_PREFIX + name); } catch(e){} }
